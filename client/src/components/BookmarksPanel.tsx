        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">{bookmarks.length} saved section{bookmarks.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {bookmarks.length > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all font-medium"
              >
                <Trash2 size={12} />
                Clear all
              </button>
        ...
        <div className="p-4">
          {bookmarks.length === 0 ? (
            <div className="text-center py-12">
              <Bookmark size={32} className="mx-auto text-gray-200 dark:text-gray-700 mb-3" />
              <p className="text-gray-500 dark:text-gray-400 font-medium text-sm">No bookmarks yet</p>
              <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">Click the bookmark icon on any section to save it here</p>
            </div>
          ) : (
                    ...
                    {items.map(bm => (
                      <div key={bm.id} className="flex items-start gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700 bg-white dark:bg-gray-800/50 group transition-all">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {bm.subtitle && <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 truncate">{bm.subtitle}</div>}
                          <div className="text-[10px] text-gray-300 dark:text-gray-600 mt-1">{new Date(bm.ts).toLocaleDateString()}</div>
                        </div>
