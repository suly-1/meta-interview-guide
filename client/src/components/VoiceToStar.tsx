...
import { Mic, MicOff, Square, Loader2, Star, ChevronDown, ChevronUp, Sparkles, Volume2 } from "lucide-react";
...
export default function VoiceToStar() {
  ...
  const transcribeAndStructure = trpc.collab.transcribeAndStructure.useMutation();
  ...
  const processVoice = useCallback(async () => {
    ...
    try {
      ...
      const result = await transcribeAndStructure.mutateAsync({ audioUrl: url, question: activeQuestion });
  ...
  const handleScore = useCallback(async () => {
    ...
    try {
      const result = await scoreAnswer.mutateAsync({ question: activeQuestion, answer: answerText });
  ...
  const isProcessing = uploadAudio.isPending || transcribeAndStructure.isPending;
      ...
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between group">
        <div className="flex items-center gap-2">
          ...
          <div className="text-left">
            <div className="text-sm font-bold text-foreground">Voice-to-STAR Recorder</div>
            <div className="text-xs text-muted-foreground">Record your answer → auto-transcribe → structure into STAR → AI score</div>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => setMode("voice")}
              className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 ${
                mode === "voice" ? "bg-rose-500/20 border border-rose-500/30 text-rose-400" : "bg-secondary text-muted-foreground hover:text-foreground"
              }`}>
              <Mic size={12} /> Voice
            </button>
            <button onClick={() => setMode("text")}
              className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 ${
                mode === "text" ? "bg-blue-500/20 border border-blue-500/30 text-blue-400" : "bg-secondary text-muted-foreground hover:text-foreground"
              }`}>
          ...
          <div className="space-y-2">
            ...
            {!useCustom ? (
              <select value={selectedQ} onChange={e => setSelectedQ(e.target.value)}
                ...
                {BEHAVIORAL_QUESTIONS.map(q => (
                  <option key={q.id} value={q.q}>{q.q.length > 80 ? q.q.slice(0, 80) + "…" : q.q}</option>
                ))}
              </select>
            ) : (
            ...
            {activeQuestion && (
              <div className="p-2.5 rounded-lg bg-rose-500/5 border border-rose-500/10 text-xs text-foreground">
                <span className="text-rose-400 font-semibold">Q: </span>{activeQuestion}
              </div>
            )}
          </div>
              ...
              {audioUrl && !recording && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2 p-2.5 rounded-lg bg-secondary">
                    ...
                    <audio src={audioUrl} controls className="flex-1 h-7" style={{ filter: "invert(0.8) hue-rotate(180deg)" }} />
                  </div>
                  <button onClick={processVoice} disabled={isProcessing}
                    ...
                    {isProcessing ? <><Loader2 size={12} className="animate-spin" /> Transcribing & Structuring…</> : <><Sparkles size={12} /> Transcribe & Structure into STAR</>}
                  </button>
                </div>
              )}
            </div>
          ...
          {mode === "text" && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={processText} disabled={transcribeAndStructure.isPending}
                  ...
                  {transcribeAndStructure.isPending ? <><Loader2 size={12} className="animate-spin" /> Structuring…</> : <><Sparkles size={12} /> Structure into STAR</>}
                </button>
              </div>
            </div>
          )}
          ...
          {starResult && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-foreground">STAR Structure</span>
                <div className="h-px flex-1 bg-border" />
              </div>
              {[
                { label: "S — Situation", value: starResult.situation, color: "text-blue-400 border-blue-500/20 bg-blue-500/5" },
                { label: "T — Task", value: starResult.task, color: "text-amber-400 border-amber-500/20 bg-amber-500/5" },
                { label: "A — Action", value: starResult.action, color: "text-emerald-400 border-emerald-500/20 bg-emerald-500/5" },
                { label: "R — Result", value: starResult.result, color: "text-purple-400 border-purple-500/20 bg-purple-500/5" },
              ].map(({ label, value, color }) => (
                <div key={label} className={`p-3 rounded-lg border ${color}`}>
                  <div className={`text-xs font-bold mb-1 ${color.split(" ")[0]}`}>{label}</div>
                  <div className="text-xs text-foreground leading-relaxed">{value || <span className="text-muted-foreground italic">Not detected</span>}</div>
                </div>
              ))}
              {starResult.rawTranscript && (
                <details className="text-xs">
                  <summary className="text-muted-foreground cursor-pointer hover:text-foreground">Raw transcript</summary>
                  <div className="mt-2 p-2.5 rounded-lg bg-secondary text-muted-foreground leading-relaxed">{starResult.rawTranscript}</div>
                </details>
              )}

              {/* Score button */}
              <button onClick={handleScore} disabled={scoreAnswer.isPending}
                ...
                {scoreAnswer.isPending ? <><Loader2 size={12} className="animate-spin" /> Scoring…</> : <><Star size={12} /> Score This Answer with AI</>}
              </button>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-secondary space-y-2">
                  <ScoreBar label="Specificity" value={scoreResult.specificity} />
                  <ScoreBar label="Impact Clarity" value={scoreResult.impactClarity} />
                </div>
                <div className="p-3 rounded-lg bg-secondary flex flex-col items-center justify-center gap-1">
                  <div className="text-2xl font-bold text-foreground">{scoreResult.icLevel}</div>
                  <div className="text-xs text-muted-foreground">Signal Level</div>
                  <div className={`badge text-xs ${
                    scoreResult.icLevel === "IC7" ? "badge-green" :
                    scoreResult.icLevel === "IC6" ? "badge-blue" : "badge-amber"
                  }`}>{scoreResult.icLevel}</div>
                </div>
              </div>
              {scoreResult.strengths && (
                <div className="p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                  <div className="text-xs font-bold text-emerald-400 mb-1">✓ Strengths</div>
                  <div className="text-xs text-foreground leading-relaxed">{scoreResult.strengths}</div>
                </div>
              )}
              {scoreResult.improvements && (
                <div className="p-3 rounded-lg bg-amber-500/5 border border-amber-500/20">
                  <div className="text-xs font-bold text-amber-400 mb-1">↑ Improvements</div>
                  <div className="text-xs text-foreground leading-relaxed">{scoreResult.improvements}</div>
                </div>
              )}
              <div className="p-3 rounded-lg bg-secondary">
                <div className="text-xs font-bold text-foreground mb-1">Coaching Note</div>
                <div className="text-xs text-muted-foreground leading-relaxed">{scoreResult.coachingNote}</div>
              </div>
            </div>
          )}
        </div>
