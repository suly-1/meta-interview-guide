...
import {
  Mic, MicOff, Square, Loader2, ChevronDown, ChevronUp,
  CheckCircle, AlertTriangle, Star, Volume2, RotateCcw
} from "lucide-react";
...
type RecordingState = "idle" | "recording" | "uploading" | "scoring" | "done" | "error";
...
export function VoiceAnswerMode({ questionText, icMode = "IC6" }: VoiceAnswerModeProps) {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (blob.size > 16 * 1024 * 1024) {
          toast.error("Recording is too long (16MB limit). Please keep answers under 5 minutes.");
          setState("idle");
          return;
        }
  ...
  const scoreColor = (s: number) =>
    s >= 4 ? "bg-emerald-500" : s >= 3 ? "bg-blue-500" : s >= 2 ? "bg-amber-500" : "bg-red-500";
    ...
    <div className="rounded-xl border border-blue-500/20 bg-blue-950/10 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">Voice Answer Mode</span>
          <span className="text-xs text-muted-foreground">Record your STAR answer — AI scores it</span>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-blue-500/10">
          ...
          <div className="rounded-lg bg-background border border-border p-3 mt-3">
            <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">Question</div>
          ...
          {state === "idle" && (
            <div className="flex flex-col items-center gap-3 py-2">
              <p className="text-xs text-muted-foreground text-center">
                Click the mic to start recording your answer. Aim for 2–3 minutes covering all 4 STAR components.
              </p>
              <button
              ...
              <div className="flex items-center gap-4 text-[10px] text-muted-foreground">
                <span className="flex items-center gap-1"><Volume2 size={10} />Speak clearly</span>
                <span>·</span>
                <span>Max 5 min</span>
                <span>·</span>
          ...
          {state === "recording" && (
            <div className="flex flex-col items-center gap-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-mono text-red-400 font-bold">{formatDuration(duration)}</span>
                <span className="text-xs text-muted-foreground">Recording…</span>
              </div>
              <p className="text-xs text-muted-foreground text-center">
                Cover: <span className="text-blue-300">Situation</span> → <span className="text-blue-300">Task</span> → <span className="text-blue-300">Action</span> → <span className="text-blue-300">Result</span>
              </p>
              <button
          ...
          {(state === "uploading" || state === "scoring") && (
            <div className="flex flex-col items-center gap-2 py-4">
              ...
              <p className="text-xs text-muted-foreground">
                {state === "uploading" ? "Uploading audio…" : "Transcribing and scoring your answer…"}
              </p>
            </div>
          )}
          ...
          {state === "done" && score && transcript && (
            <div className="space-y-3">
              ...
              <div className="rounded-lg bg-background border border-border p-3">
                ...
                <p className="text-xs text-foreground leading-relaxed max-h-32 overflow-y-auto">{transcript}</p>
              </div>
              ...
              <div className="rounded-lg bg-background border border-border p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-foreground">STAR Scorecard</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <ICBadge level={score.icLevel} />
                    <span className={`text-lg font-black ${score.overallScore >= 4 ? "text-emerald-400" : score.overallScore >= 3 ? "text-blue-400" : score.overallScore >= 2 ? "text-amber-400" : "text-red-400"}`}>
                ...
                <p className="text-xs text-muted-foreground italic">"{score.verdict}"</p>

                <div className="space-y-1.5">
                  <ScoreBar label="Situation" value={score.situation} color={scoreColor(score.situation)} />
                  <ScoreBar label="Task" value={score.task} color={scoreColor(score.task)} />
                  <ScoreBar label="Action" value={score.action} color={scoreColor(score.action)} />
                  <ScoreBar label="Result" value={score.result} color={scoreColor(score.result)} />
                </div>

                <div className="text-[10px] text-muted-foreground bg-secondary/30 rounded p-2">
                  <span className="font-semibold text-foreground">STAR Structure: </span>{score.starStructure}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/20 p-2.5 space-y-1">
                  <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Strengths</div>
                  {score.strengths.map((s, i) => (
                    <div key={i} className="flex items-start gap-1">
                      <CheckCircle size={9} className="text-emerald-400 shrink-0 mt-0.5" />
                ...
                <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-2.5 space-y-1">
                  <div className="text-[10px] font-bold text-red-400 uppercase tracking-wider">Gaps</div>
                  {score.gaps.map((g, i) => (
                    <div key={i} className="flex items-start gap-1">
              ...
              <div className="rounded-lg bg-blue-500/10 border border-blue-500/20 p-3">
                <div className="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-1">Coaching Note</div>
