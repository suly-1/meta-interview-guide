...
const BEHAVIORAL_ONLY_QUESTIONS = BEHAVIORAL_QUESTIONS.filter(q => q.area !== "XFN Partnership");
...
function MiniRound({ round, question, durationSec, onComplete }: MiniRoundProps) {
  ...
  const hints: Record<Round, string[]> = {
    ...
    behavioral: ["Open with a crisp Situation (1-2 sentences)", "State your specific Task/ownership", "Detail 3+ concrete Actions you took", "Quantify the Result — numbers matter at IC7"],
  };
    ...
    <div className="rounded-xl border border-blue-500/30 bg-blue-950/20 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-blue-300">2-Week Remediation Plan</span>
        </div>
        ...
        <div className="px-4 pb-4 space-y-2">
          {plan.map((item, i) => (
            <div key={i} className="flex gap-3 text-xs">
              <span className="shrink-0 w-5 h-5 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-300 font-bold flex items-center justify-center text-[10px]">{i + 1}</span>
...
export function FullMockDaySimulator() {
  ...
  const [answers, setAnswers] = useState<Record<Round, string>>({ coding: "", sysdesign: "", xfn: "", behavioral: "" });
  const [questions, setQuestions] = useState<Record<Round, string>>({ coding: "", sysdesign: "", xfn: "", behavioral: "" });
  ...
  const startDay = useCallback(() => {
    ...
    const bqQ = BEHAVIORAL_ONLY_QUESTIONS.length > 0 ? pick(BEHAVIORAL_ONLY_QUESTIONS) : BEHAVIORAL_QUESTIONS[0];
    setQuestions({
  ...
  const handleRoundComplete = useCallback(async (answer: string) => {
    ...
    try {
      if (round === "coding") {
        ...
        result = { round, overallScore: res.overallScore, icLevel: res.icLevel, label };
      }
    } catch {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <div className="text-xs text-muted-foreground">Coding → System Design → XFN → Behavioral · ~3 hours · AI composite scorecard + 2-week plan</div>
          </div>
        </div>
        ...
        <div className="px-5 pb-5 space-y-5 border-t border-purple-500/20">
              ...
              <div className="grid grid-cols-4 gap-2">
                {ROUNDS.map((r, i) => (
                  <div key={r} className="rounded-lg bg-slate-800/60 border border-slate-700/50 p-3 text-center">
                    ...
                    <div className="text-xs font-bold text-foreground">{roundLabel[r].replace(/^[^\s]+ /, "")}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{r === "behavioral" ? "30 min" : "45 min"}</div>
                  </div>
                ))}
              </div>
              <p className="text-xs text-muted-foreground text-center">
                Simulates a full Meta interview loop. Each round is independently scored, then the AI generates a composite IC6/IC7 promotion decision and a personalised 2-week remediation plan.
              </p>
              <button
                onClick={startDay}
          ...
          {phase === "scoring" && (
            <div className="pt-8 flex flex-col items-center gap-3">
              ...
              <div className="text-xs text-muted-foreground">The AI is reviewing all 4 rounds and building your remediation plan</div>
            </div>
          )}
          ...
          {phase === "done" && (
            <div className="pt-4 space-y-4">
                ...
                <div className="rounded-xl bg-slate-900/80 border border-purple-500/30 p-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      ...(scorecard.behavioralCoaching ? [{ label: "🎯 Behavioral STAR", text: scorecard.behavioralCoaching }] : []),
                    ].map(({ label, text }) => (
              ...
              {scorecard?.remediationPlan && scorecard.remediationPlan.length > 0 && (
