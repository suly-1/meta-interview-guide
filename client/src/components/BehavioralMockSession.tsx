...
const STAR_PHASES = [
  {
    label: "S — Situation",
    hint: "Set the scene: who were the stakeholders, what was the context, what was at stake? Be specific about the org structure and your role.",
  },
  {
    label: "T — Task",
    hint: "What was YOUR specific responsibility? What did you need to achieve or resolve? Make it clear this was your problem to solve.",
  },
  {
    label: "A — Action",
    hint: "Walk through the concrete steps YOU took. Focus on your decisions, your communication strategy, how you navigated disagreement or misalignment.",
  },
  {
    label: "R — Result",
    hint: "Quantify the outcome. What changed? What was the business or team impact? What did you learn? Would you do anything differently?",
  },
];
      ...
      <div className="prep-card p-5 space-y-3">
        ...
        <div className="text-center py-8 text-muted-foreground">
          ...
          <div className="text-xs mt-1">
            Complete an XFN mock session to see your history here.
          </div>
        </div>
      </div>
    );
            ...
            <div className="flex items-center gap-3 p-3">
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold text-foreground truncate">
                  XFN Mock — {new Date(entry.date).toLocaleDateString()}
                </div>
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  ...
                  <span
                    className={`text-xs font-bold ${entry.scorecard.level === "IC7" ? "text-violet-400" : entry.scorecard.level === "IC6" ? "text-blue-400" : "text-muted-foreground"}`}
                  >
              ...
              <div className="border-t border-border p-3 space-y-3">
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {[
                    ...
                    {
                      label: "Collaboration",
                      score: entry.scorecard.collaborationScore,
                    },
                    { label: "Conflict", score: entry.scorecard.conflictScore },
                    {
                      label: "Alignment",
                      score: entry.scorecard.alignmentScore,
                    },
                    {
                      label: "Communication",
...
export function BehavioralMockSession() {
  const [view, setView] = useState<
    "entry" | "idle" | "running" | "done" | "history"
  >("entry");
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                3 XFN questions × 12 min each — STAR phase hints, timer, and AI
                evaluation
              </div>
            </div>
          </div>
          <div className="flex gap-2 shrink-0">
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg bg-teal-500/20 hover:bg-teal-500/30 border border-teal-500/30 text-teal-400 text-sm font-semibold transition-all"
            >
              Start Mock
            </button>
          </div>
    ...
    return (
      <div className="prep-card p-5 space-y-4">
        ...
        {scoreMutation.isPending && (
          <div className="p-4 rounded-xl bg-teal-500/10 border border-teal-500/30 text-center">
            <div className="text-sm text-teal-400 animate-pulse">
              🤖 AI is evaluating your XFN session…
            </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div className="p-3 rounded-lg bg-secondary border border-border text-center">
                ...
                <div
                  className={`text-xl font-black ${scorecard.level === "IC7" ? "text-violet-400" : scorecard.level === "IC6" ? "text-blue-400" : "text-muted-foreground"}`}
                >
                  {scorecard.level}
                </div>
              </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => setView("history")}
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-teal-500/20 hover:bg-teal-500/30 border border-teal-500/30 text-teal-400 text-sm font-semibold transition-all"
          >
            ...
            <button
              onClick={() => {
                const lines: string[] = [
                  ...
                  ``,
                  `## Follow-up Questions`,
                  ...scorecard.followUpQuestions.map(
                    (q, i) => `${i + 1}. ${q}`
                  ),
                ];
    ...
    <div className="prep-card p-5 space-y-4">
      ...
      <div className="flex items-center gap-2">
        {[0, 1, 2].map(i => (
          <div
            key={i}
            className={`h-1.5 flex-1 rounded-full transition-all ${i < round ? "bg-emerald-500" : i === round ? "bg-teal-500" : "bg-secondary"}`}
          />
        ))}
      ...
      <div className="flex flex-col sm:flex-row gap-5">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="p-3 rounded-lg bg-teal-500/10 border border-teal-500/20">
            <div className="text-xs text-teal-400 font-bold mb-1">
              Interview Question
            </div>
