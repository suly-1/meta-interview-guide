...
function verdictColor(v: string) {
  if (v === "Strong Hire") return "text-emerald-400 bg-emerald-500/10 border-emerald-500/30";
  if (v === "Hire") return "text-blue-400 bg-blue-500/10 border-blue-500/30";
  if (v === "Borderline") return "text-amber-400 bg-amber-500/10 border-amber-500/30";
  ...
  return (
    <div className="relative w-28 h-28 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={`font-mono text-xl font-extrabold ${urgent ? "text-red-400" : warning ? "text-amber-400" : "text-foreground"}`}>
          {fmtTime(timeLeft)}
        </span>
...
export default function MockInterviewSimulator() {
  ...
  const startSession = (icTarget: "IC5" | "IC6" | "IC7") => {
    ...
    const bqs = [...BEHAVIORAL_QUESTIONS].sort(() => Math.random() - 0.5).slice(0, 2);
      ...
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between p-5 group">
        <div className="flex items-center gap-2">
          ...
          {phase !== "setup" && phase !== "debrief" && (
            <span className="badge badge-amber animate-pulse">{phaseLabel[phase]}</span>
          )}
        </div>
          ...
          {phase !== "setup" && (
            <div className="flex border-b border-border">
              {(["coding", "bq1", "bq2", "debrief"] as const).map((p, i) => (
                <div key={p} className={`flex-1 py-2 text-center text-xs font-semibold transition-all ${
                  phase === p ? "bg-blue-600/20 text-blue-400 border-b-2 border-blue-500" :
                  ["coding","bq1","bq2","debrief"].indexOf(phase) > i ? "text-emerald-400" : "text-muted-foreground"
                }`}>
                  {phaseLabel[p]}
                </div>
              ))}
            ...
            {phase === "setup" && (
              <div className="space-y-5">
                <div className="text-sm text-muted-foreground leading-relaxed">
                  A full 45-minute mock interview: <span className="text-blue-400 font-semibold">25 min coding</span> → <span className="text-purple-400 font-semibold">10 min BQ1</span> → <span className="text-purple-400 font-semibold">10 min BQ2</span>. Problems are randomly selected. After all three sections, an LLM generates a hiring-bar debrief.
                </div>
                <div>
                  <div className="text-xs font-bold text-muted-foreground mb-3">Select your target level:</div>
                  <div className="flex gap-3">
                    {(["IC5", "IC6", "IC7"] as const).map(ic => (
                      <button key={ic} onClick={() => startSession(ic)}
                        ...
                        <div className="text-lg mb-0.5">{ic === "IC5" ? "🌱" : ic === "IC6" ? "🔥" : "⚡"}</div>
                        <div>{ic}</div>
                        <div className="text-xs text-muted-foreground group-hover:text-blue-300 mt-0.5">
                          {ic === "IC5" ? "Senior SWE" : ic === "IC6" ? "Staff SWE" : "Sr. Staff SWE"}
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {phase === "coding" && session && (
              <div className="space-y-4">
                <div className="flex items-start gap-5">
                  <CircularTimer timeLeft={timeLeft} total={PHASE_DURATIONS.coding} />
                  <div className="flex-1 space-y-2">
                    <div className="text-xs font-bold text-blue-400 uppercase tracking-wide">Coding Problem</div>
                    <div className="text-sm font-bold text-foreground leading-relaxed">{session.codingProblem}</div>
                    <div className="text-xs text-muted-foreground">Think out loud. State your approach before coding. Discuss time/space complexity.</div>
                  </div>
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground block mb-1">Your approach / pseudocode / notes</label>
                  <textarea
                    value={session.codingNotes}
                    onChange={e => setSession(s => s ? { ...s, codingNotes: e.target.value } : s)}
            ...
            {(phase === "bq1" || phase === "bq2") && session && (
              <div className="space-y-4">
                <div className="flex items-start gap-5">
                  <CircularTimer
                    timeLeft={timeLeft}
                    total={phase === "bq1" ? PHASE_DURATIONS.bq1 : PHASE_DURATIONS.bq2}
                  />
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="text-sm font-bold text-foreground leading-relaxed">
                      {phase === "bq1" ? session.bq1Question : session.bq2Question}
                    </div>
                    <div className="text-xs text-muted-foreground">Use STAR format. Be specific with metrics and impact.</div>
                  </div>
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground block mb-1">Your STAR answer</label>
                  <textarea
                    value={phase === "bq1" ? session.bq1Answer : session.bq2Answer}
                    onChange={e => setSession(s => s ? {
                      ...s,
                      [phase === "bq1" ? "bq1Answer" : "bq2Answer"]: e.target.value
                    } : s)}
                ...
                <div className="flex gap-2">
                  <button onClick={advancePhase}
                    ...
                    {phase === "bq1" ? "Next: Behavioral Q2" : "Finish & Get Debrief"}
                  </button>
                  <button onClick={() => setRunning(r => !r)}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-secondary text-sm text-muted-foreground hover:text-foreground transition-all">
                    {running ? <Square size={12} /> : <Play size={12} />}
              ...
              <div className="space-y-5">
                {debriefMutation.isPending && (
                  <div className="flex flex-col items-center py-10 gap-3">
                    ...
                    <div className="text-sm font-semibold text-foreground">Generating your debrief…</div>
                    <div className="text-xs text-muted-foreground">The LLM is reviewing your coding approach and behavioral answers</div>
                  </div>
                )}
                ...
                {debrief && (
                  <div className="space-y-5">
                    {/* Verdict banner */}
                    <div className={`p-4 rounded-xl border text-center ${verdictColor(debrief.overallVerdict)}`}>
                      <div className="text-2xl font-extrabold mb-1">{debrief.overallVerdict}</div>
                      <div className="flex justify-center gap-1">
                        {[1,2,3,4,5].map(s => (
                          <span key={s} className={s <= debrief.overallScore ? "text-amber-400" : "opacity-20"}>★</span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="p-3 rounded-lg bg-secondary border border-border">
                      <div className="text-xs font-bold text-blue-400 mb-1">IC-Level Assessment ({session?.icTarget})</div>
                      <div className="text-xs text-foreground leading-relaxed">{debrief.icLevelAssessment}</div>
                    </div>

                    {/* Score bars */}
                    <div className="space-y-3">
                      <ScoreBar label="Coding Performance" value={debrief.codingScore} color="text-blue-400" />
                      <ScoreBar label="Behavioral Performance" value={debrief.behavioralScore} color="text-purple-400" />
                    </div>

                    {/* Assessments */}
                    <div className="grid sm:grid-cols-2 gap-3">
                      <div className="p-3 rounded-lg bg-secondary border border-border">
                        <div className="text-xs font-bold text-blue-400 mb-1">💻 Coding</div>
                        <div className="text-xs text-muted-foreground leading-relaxed">{debrief.codingAssessment}</div>
                      </div>
                      <div className="p-3 rounded-lg bg-secondary border border-border">
                        <div className="text-xs font-bold text-purple-400 mb-1">🧠 Behavioral</div>
                        <div className="text-xs text-muted-foreground leading-relaxed">{debrief.behavioralAssessment}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                      {debrief.topStrengths.length > 0 && (
                        <div>
                          <div className="text-xs font-bold text-emerald-400 mb-2">✅ Top Strengths</div>
                          <ul className="space-y-1">
                            {debrief.topStrengths.map((s: string, i: number) => (
                              <li key={i} className="text-xs text-muted-foreground flex gap-2">
                      ...
                      {debrief.criticalGaps.length > 0 && (
                        <div>
                          <div className="text-xs font-bold text-red-400 mb-2">⚠️ Critical Gaps</div>
                          <ul className="space-y-1">
                            {debrief.criticalGaps.map((g: string, i: number) => (
                              <li key={i} className="text-xs text-muted-foreground flex gap-2">
                    ...
                    {debrief.nextSteps.length > 0 && (
                      <div>
                        <div className="text-xs font-bold text-amber-400 mb-2">📋 Next Steps</div>
