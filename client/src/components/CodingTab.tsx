  ...
  const avgRating = PATTERNS.filter(p => ratings[p.id]).length
    ? (
        PATTERNS.filter(p => ratings[p.id]).reduce(
          (s, p) => s + ratings[p.id],
          0
        ) / PATTERNS.filter(p => ratings[p.id]).length
      ).toFixed(1)
    : "—";
    ...
    <div className="prep-card p-4 mb-4">
      ...
      <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
        ...
        <span className="ml-auto font-medium text-foreground">
          {masteredCount}/{PATTERNS.length} mastered · {weakCount} weak · avg{" "}
          {avgRating}
        </span>
      </div>
    </div>
...
export default function CodingTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Quick Drill */}
      {showDrillMode && (
        <QuickDrill ratings={ratings} onRate={handleRate} weakOnly={weakOnly} />
      )}
        ...
        <div className="divide-y divide-border">
          {filtered.map(p => {
            ...
            return (
              <div
                key={p.id}
                className={`p-4 transition-all ${r >= 4 ? "bg-emerald-500/3" : r > 0 && r <= 2 ? "bg-red-500/3" : ""}`}
              >
      ...
      {/* Priority #1: AI-Enabled Coding Simulator */}
      <AICodingSimulator />
      ...
      <div className="prep-card p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            [
              "AI-Enabled Round",
              "One coding round now uses CoderPad with AI assistant. Use it to check syntax, not to generate solutions. Interviewers evaluate your thought process, not the AI's.",
            ],
            [
              "Speed Matters",
              "Meta expects medium problems solved in 20–25 min. Hard problems in 35 min. Practice under time pressure from week 3 onward.",
            ],
            [
              "Clean Code",
              "Use meaningful variable names. No single-letter variables except loop counters. Add a brief comment for non-obvious logic.",
            ],
            [
              "Complexity First",
              "Always state time and space complexity before starting to code. If you can't, rethink your approach.",
            ],
            [
              "Edge Cases",
            ...
            [
              "NeetCode 150",
              "Prioritize NeetCode 150 over random LeetCode grinding. Learn the pattern, not just the solution. Aim for 80%+ completion before your interview.",
            ],
          ].map(([title, desc]) => (
