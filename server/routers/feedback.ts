...
export const feedbackRouter = router({
  ...
  /** Alias for adminGetAll — used by AdminFeedback.tsx */
  getAllSiteFeedback: adminProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];
    return db.select().from(feedbackTable).orderBy(desc(feedbackTable.createdAt)).limit(500);
  }),

  /** Digest preview — returns recent new feedback items */
  ...
  /** Alias for updateStatus — used by AdminFeedback.tsx */
