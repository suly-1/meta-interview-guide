...
Return a JSON object with this exact structure:
{
  ...
  save: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.insert(sprintPlans).values({
        userId: userId ?? 0, // 0 = anonymous
        planId,
        targetLevel: input.targetLevel ?? "IC6",
        timeline: input.timeline ?? null,
        plan: [], // required field; planData holds the full AI-generated object
  ...
  getByShareToken: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        await db.update(sprintPlans)
          .set({ viewCount: (plan.viewCount ?? 0) + 1 })
          .where(eq(sprintPlans.shareToken, input.shareToken));
      } catch { /* non-critical */ }
      return {
        planId: plan.planId,
        ...
        focusPriority: plan.focusPriority,
        weakAreas: plan.weakAreas,
        viewCount: (plan.viewCount ?? 0) + 1,
